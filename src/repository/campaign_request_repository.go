...
const (
	CreateDisposableRequestTable = "CREATE TABLE IF NOT EXISTS adpost_keyspace.DisposableCampaignRequest (id text, influencer_id text, campaign_id text, status int, PRIMARY KEY(influencer_id, id));"
	CreateMultipleRequestTable = "CREATE TABLE IF NOT EXISTS adpost_keyspace.MultipleCampaignRequest (id text, influencer_id text, campaign_id text, status int, PRIMARY KEY(influencer_id, id));"
	InsertIntoDisposableRequestTable = "INSERT INTO adpost_keyspace.DisposableCampaignRequest (id, influencer_id, campaign_id, status) VALUES (?, ?, ?, ?) IF NOT EXISTS;"
	InsertIntoMultipleRequestTable = "INSERT INTO adpost_keyspace.MultipleCampaignRequest (id, influencer_id, campaign_id, status) VALUES (?, ?, ?, ?) IF NOT EXISTS;"
	...
	GetAllDisposableCampaignRequestsByInfluencer = "SELECT id, campaign_id, status FROM adpost_keyspace.DisposableCampaignRequest WHERE influencer_id = ?;"
	GetAllMultipleCampaignRequestsByInfluencer = "SELECT id, campaign_id, status FROM adpost_keyspace.MultipleCampaignRequest WHERE influencer_id = ?;"
...
func (c campaignRequestRepository) CreateDisposableCampaignRequest(ctx context.Context, request domain.DisposableCampaignRequest) error {
	return c.cassandraClient.Query(InsertIntoDisposableRequestTable, uuid.NewString(), request.InfluencerId, request.DisposableCampaign.ID, 0).Exec()
}
...
func (c campaignRequestRepository) CreateMultipleCampaignRequest(ctx context.Context, request domain.MultipleCampaignRequest) error {
	return c.cassandraClient.Query(InsertIntoMultipleRequestTable, uuid.NewString(), request.InfluencerId, request.MultipleCampaign.ID, 0).Exec()
}
...
func (c campaignRequestRepository) GetAllDisposableCampaignRequests(ctx context.Context, userId string) ([]domain.DisposableCampaignRequest, error) {
	...
	for iter.Next() {
		err := iter.Scan(&id, &campaignId, &status)
		if err != nil {
			continue
		}
		retVal = append(retVal, domain.DisposableCampaignRequest{Id: id, InfluencerId: userId, DisposableCampaign: domain.DisposableCampaign{ID: campaignId}, RequestStatus: domain.RequestStatus(status)})
	}
...
func (c campaignRequestRepository) GetAllMultipleCampaignRequests(ctx context.Context, userId string) ([]domain.MultipleCampaignRequest, error) {
	...
	for iter.Next() {
		...
		retVal = append(retVal, domain.MultipleCampaignRequest{Id: id, InfluencerId: userId, MultipleCampaign: domain.MultipleCampaign{ID: campaignId}, RequestStatus: domain.RequestStatus(status)})
	}
